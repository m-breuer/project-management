...
@section('heading')
    ...
    <div class="ms-auto btn-group">
        <a href="{{ route("project.show", $project) }}" target="_self"
           class="btn btn-dark">{{ __("Show Project") }}</a>
...
@section('content')
    <form class="row g-2" method="POST" action="{{ route("project.task.update", [$project, $task]) }}">
        @method("PATCH")
        @csrf
            ...
            <div class="form-floating">
                <select class="form-select @error('status') is-invalid @enderror" id="status"
                    ...
                    @foreach($statuses as $status)
                        <option
                            value="{{ $status->value }}" {{ $task->status->value !== $status->value ?: "selected" }}>{{ $status->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-floating">
                <select class="form-select @error('assigned_user_id') is-invalid @enderror" id="assigned_user_id"
                    ...
                    @foreach($users as $user)
                        <option
                            value="{{ $user->id }}" {{ $task->assigned_user_id !== $user->id ?: "selected" }}>{{ $user->name }}</option>
                    @endforeach
                </select>
