...
@section('heading')
    ...
    <div class="btn-group ms-auto">
        <a href="{{ route("customer.index") }}" target="_self"
           class="btn btn-dark ms-auto">{{ __("List") }}</a>
        <a href="{{ route("customer.create") }}" target="_self"
...
@section('content')
    <form class="row g-2" method="POST"
          action="{{ route("customer.update", $customer) }}">
        @method("PATCH")
        @csrf
            ...
            <div class="form-floating">
                <select class="form-select @error('salutation') is-invalid @enderror" id="salutation" name="salutation">
                    ...
                    @foreach($salutations as $salutation)
                        <option
                            value="{{ $salutation->value }}" {{ $customer->salutation->value !== $salutation->value ?: "selected" }}>{{ $salutation->name }}</option>
                    @endforeach
                </select>
