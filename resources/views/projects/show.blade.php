...
@section('heading')
    ...
    <div class="btn-group ms-auto">
        <a href="{{ route("project.edit", $project) }}" target="_self"
           class="btn btn-dark">{{ __("Edit") }}</a>
        <a href="{{ route("project.task.create", $project) }}" target="_self"
           class="btn btn-dark">{{ __("Add Task") }}</a>
    </div>
@endsection
            ...
            <table class="table">
                ...
                @foreach($project->tasks as $task)
                    <tr>
                        ...
                        <td data-duration="{{ $task->timeSpent() }}">{{ \Carbon\CarbonInterval::seconds($task->timeSpent())->cascade()->forHumans(['short' => true, 'options' => 0]) }}</td>
                        <td>
                            {{ $task->assignedUser !== null ? $task->assignedUser->name : "-" }}
                        </td>
                        <td>{{ $task->createdUser->name }}</td>
                        <td>
                            <a href="{{ route("project.task.edit", [$project, $task]) }}"
                               target="_self" class="btn btn-dark">
                                {{ __("Edit") }}
                            </a>
                            <form class="d-inline"
                                  action="{{ route("project.task.delete", [$project, $task]) }}"
                                  method="POST">
                                @method("DELETE")
                                @csrf
                            ...
                            @if($task->openTracking)
                                <form class="d-inline"
                                      action="{{ route("project.task.tracking.stop", [$project, $task, $task->openTracking]) }}"
                                      method="POST">
                                    @method("PATCH")
                                    @csrf
                            ...
                            @else
                                <form class="d-inline"
                                      action="{{ route("project.task.tracking.start", [$project, $task, $task->openTracking]) }}"
                                      method="POST">
                                    @method("POST")
                                    @csrf
